...
public struct CRRefreshDSL: CRRefreshViewProtocol {
    ...
    public func beginHeaderRefresh(manual: Bool) {
        header?.beginRefreshing(manual: manual)
    }
    ...
    @discardableResult
    func addFootRefreshWithoutData(animator: CRRefreshProtocol = NormalFooterAnimator(), handler: @escaping CRRefreshHandler) -> CRRefreshFooterView {
        let footer = CRRefreshMake.addFootRefreshTo(refresh: scroll, animator: animator, handler: handler)
        footer.setNoMoreData(to: true)
        return footer
    }
    
    public func endFootRefresh() {
        footer?.endRefreshing()
    }
    
    public func setNoMoreData(to flag: Bool) {
        footer?.setNoMoreData(to: flag)
    }
