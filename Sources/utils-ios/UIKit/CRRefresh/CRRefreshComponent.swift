...
open class CRRefreshComponent: UIView {
    ...
    open var endRefreshingHandler: CRRefreshHandler?
    ...
    lazy var isIgnoreObservingCount: BoolCounter = {
        return BoolCounter(object: self, keyPath: \CRRefreshComponent.isIgnoreObserving)
    }()
    
    fileprivate(set) var isRefreshing = false
    fileprivate(set) var isRefreshStopping = false
    ...
    //MARK: Public Methods
    public final func beginRefreshing(manual: Bool) -> Void {
    ...
    public final func endRefreshing() -> Void {
        guard isRefreshing, !isRefreshStopping else { return }
...
//        }
        ...
        if ignore {
            isIgnoreObservingCount++
        } else {
            isIgnoreObservingCount--
        }
    }
    
    public func start(manual: Bool) { }
    
    public func stop() {
        endRefreshingHandler?()
        
        isRefreshing = false
        isRefreshStopping = false
    }
